...
[cite_start]El sistema está construido bajo una arquitectura distribuida y descentralizada utilizando Spring Cloud para solucionar problemas de escalabilidad en la gestión de adopciones[cite: 2, 10]:
* [cite_start]**apiGateway (Puerto 8090):** Centralizador de accesos, manejo de puertos dinámicos y enrutamiento de peticiones a través de predicados de ruta[cite: 44, 59, 62].
* [cite_start]**eureka (Puerto 8761):** Servidor de descubrimiento que registra dinámicamente las instancias activas de cada servicio[cite: 45, 57].
...
[cite_start]La transferencia coherente de datos y las reglas críticas del negocio no funcionan de forma aislada, sino mediante comunicación síncrona usando **OpenFeign**[cite: 46, 48]:
...
[cite_start]Para validar el funcionamiento real del sistema, se deben levantar los servicios en las consolas locales siguiendo este orden estricto[cite: 9, 21]:
1. [cite_start]**eureka** (Servidor de descubrimiento activo) [cite: 57]
2. [cite_start]**apiGateway** (Enrutador perimetral operativo) [cite: 59]
3. [cite_start]**Microservicios Core** (`usuarios`, `mascotas`, `refugios`, `blackList`, `fichaVet` y `solicitud`) [cite: 21]
